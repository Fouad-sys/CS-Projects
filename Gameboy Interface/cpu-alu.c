...
int cpu_dispatch_alu(const instruction_t* lu, cpu_t* cpu)
{
    ...
    case INC_HLR: {
		...
		cpu_write16_at_HL(cpu, cpu->alu.value);
    } break;
    ...
    case ADD_HL_R16SP: {
		addr_t address = cpu_reg_pair_SP_get(cpu, extract_reg_pair(lu->opcode));
		alu_add16_high(&cpu->alu, address, cpu_HL_get(cpu));
		cpu_HL_set(cpu, cpu->alu.value);
		cpu_combine_alu_flags(cpu, CPU, CLEAR, ALU, ALU);
    } break;

    case INC_R16SP: {
		if ((extract_reg_pair(lu->opcode) & REG_AF_CODE) == REG_AF_CODE) {
			alu_add16_high(&cpu->alu, cpu->SP, 1);
			cpu->SP = cpu->alu.value;
		}
		else {
			alu_add16_high(&cpu->alu, cpu_reg_pair_get(cpu, extract_reg_pair(lu->opcode)), 1);
			cpu_reg_pair_set(cpu, extract_reg_pair(lu->opcode), cpu->alu.value);
		}
    } break;
    ...
    case ROT_R8: {
		alu_carry_rotate(&cpu->alu, cpu_reg_get(cpu, extract_reg(lu->opcode, 0)), extract_rot_dir(lu->opcode), cpu->F);
		cpu_reg_set(cpu, extract_reg(lu->opcode, 0), cpu->alu.value);
		M_EXIT_IF_ERR(cpu_combine_alu_flags(cpu, ROT_FLAGS_SRC));
    } break;


    // BIT TESTS (and set)
	case BIT_U3_R8: {
		bit_t answer = bit_get(cpu_reg_get(cpu, extract_reg(lu->opcode, 0)), extract_n3(lu->opcode));
		flags_t res_f = 0;
    ...
    case CHG_U3_R8: {
		data_t inter = cpu_reg_get(cpu, extract_reg(lu->opcode, 0));
		do_set_or_res(lu, &inter);
		cpu_reg_set(cpu, extract_reg(lu->opcode, 0), inter);
    } break;
