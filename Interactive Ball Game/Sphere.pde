class Sphere {
  ...
  void update(float rotX, float rotZ) {
    ...
    if(milestone3.PLATE_SIZE > location.z && location.z > -milestone3.PLATE_SIZE && 
       milestone3.PLATE_SIZE > location.x && location.x > -milestone3.PLATE_SIZE) {
  ...
  void checkXEdges() {
    if (location.x > (milestone3.PLATE_SIZE)) {
      velocity.x *= -1;
    }
    else if (location.x < (-milestone3.PLATE_SIZE)) {
      velocity.x *= -1;
    }
  }
  
  void checkZEdges() {
    if (location.z > (milestone3.PLATE_SIZE)) {
      velocity.z *= -1;
    } else if (location.z < (-milestone3.PLATE_SIZE)) {
      velocity.z *= -1;
    }
  }
}
